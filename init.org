...
- Completion and navigation frameworks
- Git integration (Magit)
- Org mode configuration
- LSP and language-specific support
...
;; and keybindings similar to Spacemacs.
...
(use-package general
  ...
  ;; Global keybindings similar to Spacemacs
  (my-leader-def
...
** Helpful
Enhanced help buffers with more contextual information.
...
refactoring) for many programming languages.
...
(use-package lsp-mode
  ...
  (setq lsp-log-io nil)  ; Disable I/O logging for performance
...
(use-package lsp-ui
  ...
  :custom
  (lsp-ui-doc-enable nil)  ; Disable doc popups for performance (use K instead)
  (lsp-ui-doc-show-with-cursor nil)
  (lsp-ui-sideline-enable nil)  ; Disable sideline for performance
  (lsp-ui-sideline-show-hover nil)
  (lsp-ui-sideline-show-diagnostics nil)
  (lsp-ui-sideline-show-code-actions nil)
  (lsp-ui-peek-enable t)  ; Keep peek for go-to-definition
...
** C/C++ Configuration
Configure clangd as the LSP server for C and C++ development. Clangd provides IDE features
like code completion, go-to-definition, diagnostics, and refactoring.

This configuration supports both regular modes (c-mode, c++-mode) and tree-sitter modes
(c-ts-mode, c++-ts-mode).

#+begin_src emacs-lisp
;; C/C++ compile function using compile_commands.json
(defun c/compile-current-file ()
  "Compile the current C/C++ file using the command from compile_commands.json."
  (interactive)
  (let* ((project-root (locate-dominating-file default-directory "compile_commands.json"))
         (compile-commands-file (when project-root
                                  (expand-file-name "compile_commands.json" project-root)))
         (current-file (buffer-file-name)))
    (if (and compile-commands-file (file-exists-p compile-commands-file))
        (let* ((json-object-type 'hash-table)
               (json-array-type 'list)
               (json-key-type 'string)
               (compile-db (json-read-file compile-commands-file))
               (file-entry (seq-find (lambda (entry)
                                      (string= (gethash "file" entry)
                                              (file-name-nondirectory current-file)))
                                    compile-db))
               (compile-command (when file-entry
                                 (gethash "command" file-entry))))
          (if compile-command
              (compile compile-command)
            (message "No compile command found for %s in compile_commands.json"
                    (file-name-nondirectory current-file))))
      (message "compile_commands.json not found in project root"))))
...
(with-eval-after-load 'lsp-mode
  ...
  ;; Register language IDs for both regular and tree-sitter modes
  ...
  ;; Ensure clangd is enabled
  (unless (member 'clangd lsp-enabled-clients)
    (add-to-list 'lsp-enabled-clients 'clangd)))
#+end_src

** Typst Configuration
Typst is a modern markup-based typesetting system. This configuration adds support
for Typst with LSP integration using the tinymist language server.

You'll need to install the tinymist language server:
#+begin_example
...
#+begin_src emacs-lisp
;; Typst mode with tree-sitter support
...
;; Typst compile function
(defun typst-compile ()
  "Compile the current Typst file using the typst compiler."
  (interactive)
  (let ((filename (buffer-file-name)))
    (if filename
        (compile (format "typst compile %s" (shell-quote-argument filename)))
      (message "Buffer is not visiting a file"))))

;; Configure Typst LSP client
(with-eval-after-load 'lsp-mode
...
** JSON Configuration
Enable JSON support with tree-sitter.

#+begin_src emacs-lisp
;; JSON mode - tree-sitter will handle this automatically via treesit-auto
;; LSP hook is already configured in lsp-mode section above
#+end_src

* Tree-sitter Configuration

Tree-sitter provides fast, incremental parsing for syntax highlighting and code analysis.
This configuration automatically manages tree-sitter grammars for the languages you use.

NOTE: Emacs 30.1 expects tree-sitter ABI version 14. We pin Python, C, and C++
grammars to v0.20.x for compatibility. Newer versions (v0.21+) use ABI 15 which
causes version-mismatch errors.

#+begin_src emacs-lisp
;; Configure tree-sitter language sources
(with-eval-after-load 'treesit
  ;; Set language sources with compatible versions
  (setq treesit-language-source-alist
        '((python . ("https://github.com/tree-sitter/tree-sitter-python" "v0.20.4" "src"))
          (json . ("https://github.com/tree-sitter/tree-sitter-json" "master" "src"))
          (c . ("https://github.com/tree-sitter/tree-sitter-c" "v0.20.8" "src"))
          (cpp . ("https://github.com/tree-sitter/tree-sitter-cpp" "v0.20.5" "src")))))

;; Tree-sitter auto installation
...
** Treemacs
A file tree sidebar for project navigation.
